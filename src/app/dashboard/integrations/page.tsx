...
export default async function IntegrationsPage() {
  ...
  return (
    <>
      <PageHeader
        title="Integrations"
        description="Connect the services Ren agents work with. GitHub powers the repository workflow across your account; Supabase is connected per-project."
      />

      <div className="space-y-6">
        {/* Supabase is now per-project */}
        <Panel
          title={
            <span className="flex items-center gap-2">
              <Database className="size-3.5 text-brass" />
              Supabase backend — connected per project
            </span>
          }
        >
          <p className="text-[13px] leading-relaxed text-dusk-muted">
            Each project can connect its own Supabase backend, so agents always
            work against the right database. Open a project and use the{" "}
            <span className="font-medium text-dusk">Backend</span> panel to
            connect its Supabase project. Keys are validated, encrypted at rest,
            and the service-role key is never exposed to the browser.
          </p>
        </Panel>
